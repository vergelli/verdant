return function(H)
  ...
  H.ability_names = { [7701] = "Big Heal", [7702] = "Small Heal", [8801] = "Ward" }
  ...
  ok(n == 3, "three contributions expected (two heals, one shield), got " .. tostring(n))
  ok(rows[1].id == 7701 and rows[1].ch == 0, "Big Heal must rank first")
  ok(rows[2].id == 7702 and rows[2].ch == 0, "Small Heal must rank second")
  ok(rows[3].id == 8801 and rows[3].ch == 1, "Ward must rank last as a shield")
  ok(rows[1].v > rows[2].v and rows[2].v > rows[3].v, "rows must be sorted by value")
  local ratio = rows[1].v / rows[2].v
  ok(ratio > 2.5 and ratio < 3.5, "Big Heal must weigh about three Small Heals, got " .. tostring(ratio))
  ...
  ok(ln >= 1 and lrows[1].id == 7701, "a library session must render the view from its saved shares")
